...
Eligibility filters are in eligibility.py.
"""
...
# Re-export so callers can still import from screener.cleaner
from screener.eligibility import apply_eligibility_filters  # noqa: E402, F401
...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    """Full cleaning: normalize columns, cast numerics, dedup, quality flags."""
    df = df.copy()
...
def _flag_data_quality(df: pd.DataFrame) -> pd.DataFrame:
    """Add data_quality column flagging structural issues."""
...
def winsorize_ratios(df: pd.DataFrame) -> pd.DataFrame:
    """Cap extreme ratio values before scoring. Standard quant practice."""
    df = df.copy()
